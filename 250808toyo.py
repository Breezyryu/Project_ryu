        ...
        with tqdm(total=len(data_files), desc=f"채널 {channel} 파일 처리", unit="파일") as pbar:
            for file_name in data_files:
                if self.data_type == 'pne':
                    ...
                    df = self.parse_toyo_data_file(file_path)
        ...
        with tqdm(total=total_files, desc="파일 저장", unit="파일") as pbar:
            for channel in self.channels:
                ...
                if not self.channels[channel].empty:
                    if file_format == 'csv':
                        file_path = output_dir / f"{self.data_type}_channel_{channel}_{self.capacity_info}_data.csv"
                        self.channels[channel].to_csv(str(file_path), index=False, encoding='utf-8-sig')
                    elif file_format == 'excel':
                        file_path = output_dir / f"{self.data_type}_channel_{channel}_{self.capacity_info}_data.xlsx"
                        self.channels[channel].to_excel(str(file_path), index=False)
                    elif file_format == 'pickle':
                        file_path = output_dir / f"{self.data_type}_channel_{channel}_{self.capacity_info}_data.pkl"
                        self.channels[channel].to_pickle(str(file_path))
                    
                    pbar.set_postfix({"저장 중": f"채널_{channel}_데이터"})
                    pbar.update(1)
                
                # 용량 로그 저장 (Toyo만)
                if not self.capacity_logs[channel].empty:
                    if file_format == 'csv':
                        file_path = output_dir / f"{self.data_type}_channel_{channel}_{self.capacity_info}_capacity.csv"
                        self.capacity_logs[channel].to_csv(str(file_path), index=False, encoding='utf-8-sig')
                    elif file_format == 'excel':
                        file_path = output_dir / f"{self.data_type}_channel_{channel}_{self.capacity_info}_capacity.xlsx"
                        self.capacity_logs[channel].to_excel(str(file_path), index=False)
                    elif file_format == 'pickle':
                        file_path = output_dir / f"{self.data_type}_channel_{channel}_{self.capacity_info}_capacity.pkl"
                        self.capacity_logs[channel].to_pickle(str(file_path))
                    
                    pbar.set_postfix({"저장 중": f"채널_{channel}_용량로그"})
                    pbar.update(1)
